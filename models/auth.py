...
class Account:
  ...
  def __repr__(self):
    return f"Account(username='{self.username}', roles={self.roles}, status='{self.status}')"
...
class CLIAuthenticator:
  ...
  def register(self, account: Account) -> bool:
    ...
    if result:
      return False # Usernamealready exists
    ...
    # Insert acccount-role relationship into the Account_Role table
  ...
  def authenticate(self, username: str, password: str) -> Account:
    ...
    if result and result[2] == password: # Check if the password matches
      account_id = result[0]
      ...
      return Account(username=result[1], 
                     password=result[2], 
